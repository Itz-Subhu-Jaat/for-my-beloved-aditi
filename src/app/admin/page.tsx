...
import { Heart, Upload, X, ArrowLeft, ImageIcon, LogOut } from 'lucide-react'
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#fdf2f8] flex items-center justify-center p-4">
          ...
          >
            <div className="glass rounded-3xl p-8 text-center" style={{
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                />
                {error && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-400/70 text-sm">
                    {error}
                  </motion.p>
                )}
                <button
          ...
          >
            ...
            <div className="glass rounded-2xl p-6 mb-8" style={{
              ...
              <button
                ...
                disabled={uploading}
                className="w-full py-8 rounded-xl border-2 border-dashed border-pink-500/20 hover:border-pink-500/40 flex flex-col items-center gap-3 text-pink-400/60 hover:text-pink-300 transition-all disabled:opacity-50"
              >
                <Upload className="w-8 h-8" />
                <span className="text-sm font-medium">
                  {uploading ? 'Uploading...' : 'Click to upload photos'}
                </span>
                <span className="text-xs text-pink-400/30">Supports JPG, PNG, WebP</span>
              </button>
            </div>
